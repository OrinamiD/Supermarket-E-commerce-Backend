...
const handlePlaceAnOrder = async (req, res)=>{
    ...
    return res.status(200).json({message: "New order successful", newOrder, updatedProduct: existingProduct})
...
const handleUpdateOrderStutus = async (req, res)=>{
  ...
  try {
    ...
    return res.status(200).json({message: "Status updated successfully", updateStatus})
...
const handleGetAllOrders = async (req, res)=>{
 ...
 try {
  ...
  if(!orderDetails){
    return res.status(404).json({message: "Order details not found. Carry out a new order"})
  }
